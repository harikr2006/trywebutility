...
export default function Base64Page() {
  ...
  function handleEncode() {
    const { output: out, error: err } = encodeBase64(input);
    setOutput(out);
    setError(err);
    setMode("encode");
  }
  ...
  function handleDecode() {
    const { output: out, error: err } = decodeBase64(input);
    setOutput(out);
    setError(err);
    setMode("decode");
  }
  ...
  return (
    <ToolShell
      ...
      description="Encode text to Base64 or decode Base64 strings back to text. All processing in your browser."
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between h-7">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
              {mode === "encode" ? "Base64 Output" : "Output"}
            </label>
